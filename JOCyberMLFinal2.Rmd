...
We can see that output of the random forest model is great on test set. The overall accuracy of the model is 97% and specificity of the model is 94%. This result is great and we'll use this model over others. 


# Conclusion

Through this analysis, we were able to confirm that machine learning approaches to the prediction of Malicious URLs are helpful as they can decrease the risk of cyber crime and theft. In our analysis, random forest model was able to secure a 94% Malicious URL prediction rate, which serves as a testament. Other advanced approaches using neural network can be more accurate in prediction. We saw that length of the URL and number of special characters in a URL are important when it comes to classification. 
...
1) Applying gradient boosting (through XGBoost) for regression purposes.
