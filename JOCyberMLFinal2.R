...
# We can see that WHOIS_COUNTRY has different values for one country and that needs to be corrected.
...
# This could be overfitting/leakage but for current purposes
...
# Now, we'll look at the structure of the chardset variable: 
unique(train$CHARSET)

# We can see similar problem in this variable as in WHOIS_COUNTRY. 
...
# Let's plot the DNS_Query_times with target variables. 
...
# Is URL_length a good predictor for type of url link. 
